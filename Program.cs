...
void ErrorArray(int[,] inArray, int rowPos, int columnPos)
{
    if (row > inArray.GetLength(0) || str > inArray.GetLength(1))
    {
        Console.WriteLine("Числа с таким индексом в массиве нет!");
    }
}

Console.WriteLine($"Искомое число - " + array[str, row]);
